...
function loadNextImage() {
    ...
    if (images.length === 0) {
        // Brak obrazów - symulacja ładowania
        simulateLoading();
    } else {
        // Są obrazy do załadowania
...
function simulateLoading() {
    const fakeTotal = 100;
    total_images.value = fakeTotal;
    number_of_images_loaded.value = 0;

    const interval = setInterval(() => {
        number_of_images_loaded.value++;
        if (number_of_images_loaded.value >= fakeTotal) {
            clearInterval(interval);
        }
    }, DELAY_BETWEEN_PHOTO_LOADED);
}
